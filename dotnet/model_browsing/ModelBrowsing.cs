                ...
                foreach (string ldName in serverDirectory)
                {
                    ...
                    foreach (string lnName in lnNames)
                    {
                        ...
                        foreach (string dataObject in dataObjects)
                        {
                            ...
                            {
                                string daReference = logicalNodeReference + "." + dataObject + "." +
                                                     ObjectReference.getElementName(dataDirectoryElement);

                                // get the type specification of a variable
                                MmsVariableSpecification specification = con.GetVariableSpecification(daReference,
                                    ObjectReference.getFC(dataDirectoryElement));
                                ...
                                if (specification.GetType() == MmsType.MMS_STRUCTURE)
                                {
                                    ...
                                    {
                                        Console.WriteLine("           " + elementSpec.GetName() + " : " +
                                                          elementSpec.GetType());
                                    }
                                }
                            }
